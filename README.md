...
- Feature engineering: `CarAge` from `Year`, binning mileage into 5 buckets (Very Low to Very High), one hot encoding brand/fuel type/transmission
...
The gap is bigger than I expected going in. Linear regression assumes price moves in a straight line with each feature, but car depreciation isn't linear. A car losing its first 20k miles hits price way different than one going from 100k to 120k, and brand/mileage/age interact with each other in ways a single linear coefficient can't capture. Random Forest picks all of that up for free, which is basically the whole lesson of this project.
